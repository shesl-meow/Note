...
| `static getDerivdStateProps(props, state) => obj|null` | 让组件在 `props` 变化时更新 `state`。                        | 每次执行渲染前都会调用。<br />如果返回值设为 `obj` 那么执行之后组件的 `state` 会被设置为 `{...this.state, ...obj}`； |
...
| 方法                                                         | 一般作用                                       | 调用时机与效果                                               |
| ------------------------------------------------------------ | ---------------------------------------------- | ------------------------------------------------------------ |
| `static getDerivdStateProps(props, state) => obj|null`       |                                                | 注意参数是更新之后的值                                       |
| `shouldComponentUpdate(nextProps, nextState) => bool`        | 决定组件是否继续执行更新过程                   | 返回的值如果是 `false`，则组件不会继续执行后续函数的更新过程（`forceUpdate` 时会跳过这个函数的执行） |
| `render`                                                     |                                                |                                                              |
| `getSnapshotBeforeUpdate(prevProps, prevState) => snapshot || null` | 需要在更新 DOM 前保存当前 DOM 一些状态值时使用 | 在最近一次渲染输出（即提交到 DOM 节点）前调用                |
