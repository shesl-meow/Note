# xorz

这个题目非常简单，因此在比赛的时候懒得计算爆破的时间复杂度而错误估计了爆破的复杂度。

实际的爆破时间复杂度为：长度的爆破 38 * flag 字符的取值空间约 64，大致为 11 个比特的爆破。爆破的依据是明文字符应该在可能的字符串字典中：

```python
from itertools import *

salt="WeAreDe1taTeam"
si = cycle(salt)
cipher = ""
cipher_lst = [int(cipher[n:n+2], 16) for n in range(0, len(cipher), 2)]

keyXOplain = [c ^ ord(next(si)) for c in cipher_lst]

flag_dic = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_'
plain_dic = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ,. '
possible_length = {}
for length in range(38)[1:]:
    this_key= []
    for i in range(length):
        key_bytes = []
        for f in flag_dic:
            res = [chr(keyXOplain[j] ^ ord(f)) in plain_dic for j in range(i, len(keyXOplain), length)]
            if sum(res) != len(res): continue
            key_bytes.append(f)
        this_key.append(key_bytes)
    this_key = ''.join([(
        ''.join(k) if len(k)==1 else '?' if len(k)==0 else '<'+''.join(k)+'>'
    ) for k in this_key])
    possible_length[length] = this_key

from json import dumps
print(dumps(possible_length, indent=2))
```

